...
final class LH_MCP_General_Abilities_Post_Read_Support {
	 ...
	 * carries no serialized block comments - that is, it is classic
	 * pre-block-editor HTML with no blocks to convert. That case is reported
	 * as its own source, 'classic', because nothing is broken and there is
	 * nothing for anyone to fix; treating it as breakage sends the caller
	 * chasing a non-existent problem. Any other diagnostic has its message
	 * passed through verbatim, since the transformer's own account of what
	 * went wrong is more useful than a generic one.
	 *
		...
		if ( 'success' !== ( $result['status'] ?? '' ) ) {
			...
			if ( in_array( 'format_bridge_validation_failed', $codes, true ) ) {
				return array(
					'content' => null,
					'source'  => 'classic',
					'notice'  => 'This post predates the block editor: content_raw is plain HTML with no serialized block comments, so there are no blocks to convert to Markdown. Nothing is broken, nothing needs installing or activating, and there is nothing here to report as a problem. Read and edit content_raw directly as HTML.',
				);
			}
