 ...
 * on: 'classic' means the post has no block markup and there is nothing to
 * convert, which is not a fault, while 'unavailable' and 'failed' are.
 *
	...
	public static function output_schema(): array {
		return array(
			...
			'properties' => array(
				...
				'content_raw'      => array( 'type' => 'string', 'description' => 'The post\'s stored content: Gutenberg block markup for a block-editor post, or whatever was saved without blocks for a classic one - HTML, plain text, shortcodes or a bare embed URL. Always complete, never affected by markdown conversion.' ),
				...
				'markdown_source'  => array(
					...
					'description' => 'Where content_markdown came from. "classic" means the post has no block markup to convert, as with classic-editor content or anything else saved without blocks, whatever its age — expected, not an error. "unavailable" means the transformer plugin is not active; "failed" means it errored.',
				),
				'notice'           => array(
					'type'        => 'string',
